...
const About = () => {
  ...
  const [nftMintedForAddress, setNftMintedForAddress] = useState<
    boolean | null
  >(null); // should re
  const [
    twitterHandleRetrievedFromContract,
    setTwitterHandleRetrievedFromContract,
  ...
  const {
    ...
    isFetching: f1,
    error: e1,
  } = useMoralisWeb3ApiCall(getNativeBalance); // https://github.com/MoralisWeb3/react-moralis#resolve-data-with-usemoralisweb3apicall mask.
  const {
    data: d2,
    isFetching: f2,
    error: e2,
  } = useMoralisWeb3ApiCall(getTokenBalances);
  ...
  const twitterVerification = (
    <RaisedPaperCode>
      <div>
        <h3>
          Connect your Twitter Handle to your account by tweeting
          "Verification":
        </h3>
        <TextField
          id="outlined-basic"
        ...
        />
        <br />
        <Button variant="contained" onClick={validateTwitterData}>
          Connect Twitter Handle To Profile
        </Button>
      </div>
    </RaisedPaperCode>
  );
  return (
    <>
      ...
      <div>
        <Button variant="contained" onClick={checkIfNFTMintedForUser}>
          Check if Address Minted (note, on Ropsten, needed to wait a few
          seconds for miners. )
        </Button>
        {nftMintedForAddress !== null && (
          <>
            <h5>NFT Minted: </h5>
            {nftMintedForAddress ? 'yes, minted' : 'no, not minted'}{' '}
          </>
        )}
      </div>
      ...
      <div>
        <Button variant="contained" onClick={loadTwitterHandle}>
          Read Twitter Handle from Contract
        </Button>
        {twitterHandleRetrievedFromContract !== null && (
          <>
